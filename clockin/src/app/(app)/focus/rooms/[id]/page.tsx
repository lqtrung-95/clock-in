...
import { ImmersiveBoundary } from "@/components/shell/immersive-boundary";
import {
      ...
      <div className="max-w-md w-full p-8 text-center space-y-6 bg-white/5 backdrop-blur-2xl border border-white/10 rounded-3xl shadow-2xl">
        ...
        <div className="space-y-3">
          ...
          <Button
            ...
            onClick={() => router.push("/progress/leaderboard")}
          >
            Back to Social
          </Button>
        </div>
...
function FocusRoomPageInner() {
  ...
  const confirmLeave = async () => {
    ...
    router.push("/progress/leaderboard");
  };
...
// Wrapped rather than restructured — the 907-line body above manages
// realtime Supabase subscriptions directly; moving those is Phase 3 work.
// This wrapper only hides the app shell chrome while a room is open, the
// same treatment /focus's active session gets, so the shell's sticky
// topbar doesn't collide with this page's own h-screen layout.
