-- Dream Goal Visualization tables
-- Migration 00016: Create dream_goals and dream_goal_progress_history tables
...
-- Indexes
CREATE INDEX IF NOT EXISTS idx_dream_goals_user_id ON public.dream_goals(user_id);
CREATE INDEX IF NOT EXISTS idx_dream_goals_theme ON public.dream_goals(theme);

-- RLS Policies
ALTER TABLE public.dream_goals ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Users can view own dream goal" ON public.dream_goals;
CREATE POLICY "Users can view own dream goal"
  ON public.dream_goals FOR SELECT
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can create own dream goal" ON public.dream_goals;
CREATE POLICY "Users can create own dream goal"
  ON public.dream_goals FOR INSERT
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update own dream goal" ON public.dream_goals;
CREATE POLICY "Users can update own dream goal"
  ON public.dream_goals FOR UPDATE
  USING (auth.uid() = user_id);

-- ============================================
-- Dream Goal Progress History Table
-- ============================================

CREATE TABLE IF NOT EXISTS public.dream_goal_progress_history (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- Indexes
CREATE INDEX IF NOT EXISTS idx_dream_goal_history_goal_id ON public.dream_goal_progress_history(dream_goal_id);
CREATE INDEX IF NOT EXISTS idx_dream_goal_history_created_at ON public.dream_goal_progress_history(created_at);

-- RLS Policies
ALTER TABLE public.dream_goal_progress_history ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Users can view own progress history" ON public.dream_goal_progress_history;
...
DROP POLICY IF EXISTS "Users can create progress history" ON public.dream_goal_progress_history;
...
DROP TRIGGER IF EXISTS trigger_dream_goal_updated_at ON public.dream_goals;
